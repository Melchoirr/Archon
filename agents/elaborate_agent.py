...
from shared.utils.config_helpers import extract_topic_spec
...
class ElaborateAgent(BaseAgent):
    def __init__(self, topic_dir: str, output_path: str = "context.md",
        ...
        spec_content = extract_topic_spec(topic_dir)

        # 从 topic_spec.md 提取 title 和 domain
        topic_title = ""
        topic_domain = ""
        for line in spec_content.split("\n"):
            line_s = line.strip()
            if line_s.startswith("# ") and not topic_title:
                topic_title = line_s[2:].strip()
            elif line_s.startswith("## ") and ("领域" in line_s or "domain" in line_s.lower()):
                idx = spec_content.index(line) + len(line)
                rest = spec_content[idx:].strip()
                topic_domain = rest.split("\n")[0].strip() if rest else ""
        if not topic_title:
            topic_title = os.path.basename(topic_dir)
